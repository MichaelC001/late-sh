...
/// game boards (each door's board triple, then the Lateania boards), then
...
fn board_list_follows_the_rosters() {
    ...
    assert_eq!(boards[2 + door_boards], Board::LateaniaAdventurers);
    assert_eq!(boards[3 + door_boards], Board::LateaniaFrontier);
    assert_eq!(
        boards.len(),
