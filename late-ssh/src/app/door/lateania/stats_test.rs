...
fn the_damage_scores_say_when_they_tell() {
    ...
    assert!(Score::Intelligence.rule().ends_with("burst, tells from the first tick"));
    assert!(Score::Wisdom.rule().ends_with("stamina, tells once the pool runs dry"));
    assert!(Score::Dexterity.rule().ends_with("spiky, same average as STR"));
}
