...
- Last updated: 2026-09-12 (the character-select list is no longer polled: the database is read once per account (`fill_slots_task`) and every later change rides the write that made it (`persist`/`delete_character_task` -> `publish_slot`), with staged saves and the live character's world row laid over the stored rows at read time, `SlotList::Loading` as its own state, and `App::tick` turning a changed list into a frame within 500ms. See §3.)
- Status: Active
