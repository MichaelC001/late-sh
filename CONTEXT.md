...
- Last updated: 2026-09-04 (Artboard paint colour picker: `Ctrl+K` opens a modal with R/G/B bars, a hex field, and the 16 presets, Enter applies any RGB; the presets and `Ctrl+U`/`Ctrl+Y` stay as they were and the info block's palette cells are clickable; `Alt+K` and double-click sampling are the eyedropper; `late-ssh/src/app/artboard/color_picker.rs`. Page rails carry a full-height rule from the top border on Games, Artboard, and Leaderboards, like the room rail; the Artboard rail folds while the board has the keys and shows its counts on entry. Artboard page lands on its rail; the archive modal is gone: `Daily` / `Monthly` / `Curated` rows close the rail, the rail becomes the key list and the board shows the key under the cursor, keys listed once and one canvas loaded at a time. Same day: Artboard gallery: pieces framed off the live board and hung with `Hang a piece` on the new Artboard page rail, one free revocable applause per person, the monthly `artboard` profile award (`ART1`-`ART3`, best piece by applause, 3 applause to count, the one ranked award that pays chips: 10k / 5k / 1k inside the snapshot transaction), last month's winner on the splash, yesterday's most applauded piece in The Late Edition, `/mod artboard remove|gallery`, `artboard_gallery_enabled` switch; migration 174, `late-ssh/src/app/artboard/CONTEXT.md` §Gallery.)
- Status: Active
...
- The gallery (`app/artboard/gallery`): in view mode a rail on the left of the page (the page lands on it; Esc from the board goes back to it) lists the live board, the gallery listings (This month, New, Hall of fame, Mine), `Hang a piece`, and the archive browser. Hanging crops a frame of the live board into an immutable `artboard_pieces` row: at least 40 glyphs, at most 100x40, at least 75% painted by the hanger per cell provenance, three a day, no exact repeat in a month (content hash). Applause (`v`) is one per person per piece, free, revocable, never on your own. Month end: the `artboard` profile award ranks hangers by their best piece's applause (3 to count, earliest hang breaks a tie, `ROW_NUMBER` so a tie never pays twice) and pays 10,000 / 5,000 / 1,000 chips once, in the award snapshot's transaction; the month is settled by the first pass after the rollover and later passes pay nothing. The switch also blanks the splash piece and the paper's wall column. Last month's winner hangs on the splash; yesterday's most applauded piece prints in The Late Edition. Switch `artboard_gallery_enabled` (`/mod artboard gallery on|off`); `/mod artboard remove <id-prefix>` takes a piece down. The web `/gallery` does not list pieces yet.
