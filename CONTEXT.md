...
Deploy workflows (`deploy.yml`, `deploy_web.yml`, `deploy_infra.yml`, `deploy_cli.yml`) trigger on published GitHub Releases and also support manual `workflow_dispatch` with explicit `release_tag` and `environment` inputs. Shared `ci`, `build`, and `terraform` workflow calls accept `source_ref`, so manual deploys check out the requested tag instead of the UI-selected branch. Use this as the recovery path when GitHub misses a release event. Door-game deploy workflows (`deploy_nethack.yml`, `deploy_dopewars.yml`, `deploy_dcss.yml`, `deploy_usurper.yml`, `deploy_brogue.yml`, `deploy_codekeep.yml`) are image-only: `ci` + `build`, then `kubectl set image` on that game's deployment (init and main container) and a rollout wait. They never run terraform on ordinary releases, so nothing else in the cluster is touched; the Dockerfile gives each door its own `builder-<door>` stage, so a door image build compiles only that host crate, never `late-ssh`/`late-web`. `deploy_codekeep.yml` additionally self-bootstraps (copy the pattern for future games): a `check_deployment` job reads live cluster state, and when the game's deployment is missing (first deploy, disaster recovery) the run switches to `terraform.yml` with a `targets` input scoping the apply to exactly that game's resources plus dependencies; no manual dispatch needed. Other manifest changes ship through `deploy_infra.yml`, whose `get_images` job reads every live image tag off the cluster and therefore requires all game deployments to already exist.
