...
- Last updated: 2026-08-13 (Stream audience signals: a friend going live fires a banner + `Friends` desktop notification, and the first time a named late.sh user opens a stream — `/watch @user` or walking into the stream room — it posts an `is watching` #lounge line and alerts the streamer with a banner + `GameEvents` notification. New `ActivityKind::WatchingStream`; anonymous watch-page viewers stay count-only. Domain details in `late-ssh/src/app/stream/CONTEXT.md` §3b)
- Status: Active
