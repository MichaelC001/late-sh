...
- Last updated: 2026-09-02 (**Multi-replica rule** added to §0: every new feature is designed for several `late-ssh` replicas from now on, truth in Postgres, once-only actions as DB claims, fan-out via `LISTEN/NOTIFY`, no new process-global memory. The first-contact haunting joined the §7 debt table for its in-memory kill switch and per-session daily caps; its eligibility gate shipped under that rule the same day: stage 1 universal behind the `haunt_live` fuse, stages 2-4 behind seven days of connected time (the online-time leaderboard's table, not account age) + touched settings + an AI-screened bio, every cap and stamp a conditional claim on the user row. `app/flags/svc.rs` (`app_flags` rows, migration 171) is the reusable home for any process-wide switch. `late-ssh/src/app/deadchannel/CONTEXT.md`.)
- Status: Active
...
| `late-ssh/src/app/deadchannel/CONTEXT.md` | The deadchannel game domain (GAME.md): the first-contact haunting ladder (clock glitch, name flicker, splash whisper, invitation DM), the eligibility gate (tenure, touched settings, AI-screened bio), the glyph alphabet, `/haunt` admin controls, or the haunt kill switch and fuse (`app_flags`). | Domain layout (game root + `haunt/` onboarding subdomain), the four stage machines and their chain (hits arm the whisper, the whisper schedules the invitation), the gate and its thresholds, the replica rule as applied (row claims for every cap and stamp, flags through `app/flags`), persisted `first_contact_*` marks, the `afterglow` ghost voice, render hooks, `/haunt` command surface, tests, and gotchas. |
