...
Chat badges and flags are rentals too, but they ride the chat label query rather than the flair directory: migration 148 derives a 24h and a 30-day `badge_rental` from every legacy `item_kind = 'badge'` row (100/3,000 basic, 250/7,500 premium, flags at the basic price) and retires the permanent SKUs with `active = false`. A rental writes a user-scoped effect row under `effect_kind = 'chat_badge'` / `'chat_flag'`, and `User::list_chat_author_metadata` COALESCEs a live rental over the legacy permanent equip, so pre-rental owners keep rendering what they bought whenever nothing is rented over it. `late-core/src/models/rental.rs` is the one place the two rental windows, their copy, and the payload readers live, for all four rental kinds.
