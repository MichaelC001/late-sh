...
- Last updated: 2026-07-27 (leaderboard sessions now seed from the already-published snapshot instead of waiting on the `has_changed()` gate, and a connect refreshes a snapshot already older than `REFRESH_INTERVAL`; the 300s cadence and subscriber gate are unchanged)
- Status: Active
...
**Key enums:**
- `Screen`: `Dashboard`, `Arcade`, `Games`, `Artboard`, `Lateania`, `Rebels`, `Nethack`, `Dopewars`, `GreenDragon`, `Pinstar`, `Clubhouse`, `DailyMatch`, `HouseTable` (the door-game screens — Lateania/Rebels/Nethack/Dopewars/GreenDragon — are reached only through the Games hub, and `DailyMatch`/`HouseTable` only from the Lobby modal; none are in the tab cycle. `Pinstar` renders as Directory (Profiles, Projects, and Pinstar tabs) on key `5`; `Clubhouse` is the tavern on key `0`, first in the Tab cycle and the landing screen for every session — see `late-ssh/src/app/clubhouse/`). `Dashboard` is rendered as Home and owns the chat room rail/center. News, Mentions, RSS, Voice, and Discover are synthetic room-like entries within Home chat. Showcase/Projects and Work/Profiles data still use chat-adjacent services and unread cursors, but their UI lives on Directory page 5, not the Home rail or room jump picker.
...
| arrows / `h`/`l`/`j`/`k` | Games hub (page 3) | Switch the selected door-game card (Lateania / NetHack / DCSS / Brogue / Usurper / Green Dragon / Rebels / dopewars) |
